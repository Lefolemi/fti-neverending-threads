...
func populate_list(history: Array) -> void:
	...
	# 2. Clear any existing review items safely
	for child in content_vbox.get_children():
		child.queue_free()
		
	await get_tree().process_frame
	
	# 3. Create new items based on history data (with Chunking!)
	for i in range(history.size()):
		var data = history[i]
		_create_review_item(i, data)
		
		# Yield execution every 5 items to keep the framerate perfectly smooth
		if i % 5 == 0:
			await get_tree().process_frame
...
func _create_review_item(index: int, data: Dictionary) -> void:
	...
	# PASS allows the panel to be clicked but still lets the scroll wheel/drag pass through
	...
	lbl_question.autowrap_mode = TextServer.AUTOWRAP_WORD
	lbl_question.mouse_filter = Control.MOUSE_FILTER_IGNORE # Scroll Fix
	...
	lbl_yours.autowrap_mode = TextServer.AUTOWRAP_WORD
	lbl_yours.mouse_filter = Control.MOUSE_FILTER_IGNORE # Scroll Fix
	...
	if not data["is_correct"]:
		...
		lbl_right.autowrap_mode = TextServer.AUTOWRAP_WORD
		lbl_right.mouse_filter = Control.MOUSE_FILTER_IGNORE # Scroll Fix
	...
	btn_mark.toggle_mode = false 
	btn_mark.mouse_filter = Control.MOUSE_FILTER_PASS # Allow clicks but don't block drags
